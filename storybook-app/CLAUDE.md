...
### Modal _(Mobile)_
**Import:** `import { Modal } from '../components/Modal/Modal'`  
**Story:** `Mobile/Modal`

Container de modal usado em telas abertas via `router.modal()`. Visual idêntico ao `.card.cardDark` — fundo glass dark, borda glass, border-radius no topo. Header com até 3 slots: voltar (esquerda), título (centro), fechar (direita).

| Prop | Tipo | Padrão | Descrição |
|------|------|--------|-----------|
| `title` | `string` | — | Título centralizado no header |
| `description` | `string` | — | Subtítulo abaixo do título (opcional) |
| `onBack` | `() => void` | — | Exibe botão ← quando fornecido |
| `onClose` | `() => void` | — | Exibe botão ✕ quando fornecido |
| `children` | `ReactNode` | — | Conteúdo scrollável do modal |

```tsx
<Modal title="Junte-se a nós" description="Rápido e simples." onClose={() => router.closeModal()}>
  {/* campos do formulário */}
</Modal>
```

**Em telas HTML:** usar `body.modal-screen` + classes de `page-mobile.css`:
```html
<link rel="stylesheet" href="../../shared/page-mobile.css" />

<body class="layout-screen modal-screen">
  <div class="modal">
    <div class="modal-header">
      <div class="modal-header-start">
        <!-- botão voltar (opcional) -->
        <button class="modal-nav-btn" onclick="router.pop()" aria-label="Voltar">
          <i data-lucide="chevron-left" width="20" height="20"></i>
        </button>
      </div>
      <div class="modal-header-center">
        <h2 class="modal-title">Junte-se a nós</h2>
        <p class="modal-desc">Rápido e simples.</p>
      </div>
      <div class="modal-header-end">
        <button class="modal-nav-btn" onclick="router.closeModal()" aria-label="Fechar">
...
> **Partial:** `data-transition="modal"` no wrapper faz o router aplicar `.modalScreen` (70% height, bottom 0, border-radius no topo). O body `modal-screen` tem `background: transparent` para a tela anterior aparecer atrás.

---

### InputAction _(Mobile)_
**Import:** `import { InputAction } from '../components/InputAction/InputAction'`
**Story:** `Mobile/InputAction`

Input com botão de ação embutido à direita. Usado quando o preenchimento do campo dispara uma ação imediata (ex: envio de código por telefone).

| Prop | Tipo | Padrão | Descrição |
|------|------|--------|-----------|
| `label` | `string` | — | Label acima do campo |
| `actionLabel` | `string` | — | Texto do botão de ação |
| `actionSent` | `boolean` | `false` | Estado pós-envio — desabilita botão e mostra "Código enviado" |
| `onAction` | `() => void` | — | Callback ao clicar no botão |
| `onChange` | `(value: string) => void` | — | Callback ao digitar |
| `disabled` | `boolean` | — | Desabilita campo e botão |
| + todos `InputHTMLAttributes` | — | `placeholder`, `type`, `value`, etc. |
...
**Em telas HTML:** usar as classes `.iaWrapper`, `.iaLabel`, `.iaInputWrap`, `.iaInput`, `.iaActionBtn` de `InputAction.module.css`. Adicionar classe `.sent` no botão e `disabled` no elemento após envio.

```html
<link rel="stylesheet" href="../../storybook-app/src/components/InputAction/InputAction.module.css" />

<div class="iaWrapper">
  <span class="iaLabel">Seu número de telefone</span>
  <div class="iaInputWrap">
    <input class="iaInput" type="tel" placeholder="Ex.: (48) 99945-9453" />
    <button class="iaActionBtn" type="button" onclick="enviarCodigo()">Enviar código</button>
  </div>
</div>
```
...
Container de tela inteira para fluxos de autenticação. Logo centralizada e card com glass effect. Usado em todas as telas de auth.
...
| `background` | `'glass' \| 'glass-dark'` | `'glass'` | Variante de fundo do card |

**Variantes de background:**

| Valor | Token | Quando usar |
|-------|-------|-------------|
| `'glass'` | `--color-glass-surface` | Fundo sólido escuro (padrão) |
| `'glass-dark'` | `--color-glass-dark` | Sobre imagem de fundo — maior contraste |

**Em telas HTML:** aplicar a classe `cardDark` junto com `card` para a variante escura.

```tsx
{/* Padrão */}
<AuthCard title="Crie sua senha">...</AuthCard>

{/* Sobre imagem de fundo */}
<AuthCard title="Bem-vindo" background="glass-dark">...</AuthCard>
```

```html
<!-- HTML standalone — variante escura -->
<div class="card cardDark login-card">...</div>
```

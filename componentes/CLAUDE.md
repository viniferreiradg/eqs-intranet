...
### TeamMemberCard
**Import:** `import { TeamMemberCard } from '../TeamMemberCard/TeamMemberCard'`
**Story:** `Components/TeamMemberCard`

Card de colaborador — seção "Nossa equipe" de `detalhes-setor.html`. Avatar + nome/cargo/e-mail à esquerda, botão de e-mail (`mailto:`) outline à direita (padrão de icon box: borda `gray-300`, sem fundo, ícone na cor da marca). Usa `Avatar` e `Card` internamente.

| Prop | Tipo | Padrão | Descrição |
|------|------|--------|-----------|
| `name` | `string` | — | Nome do colaborador (obrigatório) |
| `role` | `string` | — | Cargo/função (obrigatório) |
| `email` | `string` | — | E-mail — também vira o `mailto:` do botão (obrigatório) |
| `avatarUrl` | `string` | — | Foto do avatar |
| `initials` | `string` | — | Fallback quando sem foto |
